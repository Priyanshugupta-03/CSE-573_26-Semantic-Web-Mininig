# -*- coding: utf-8 -*-
"""
Created on Tue Mar 17 19:34:25 2026
...
# Paste your current cookie string here
...
# Print all IDs found on the page
print("=== ALL IDs FOUND ===")
...
# Print all section-like divs with classes
print("=== ALL SECTION-LIKE CLASSES ===")
for tag in soup.find_all(["div", "section"], class_=True):
    classes = " ".join(tag.get("class", []))
    text_preview = tag.get_text(strip=True)[:60]
    if text_preview:
        print(f"  class={classes[:50]}  →  {text_preview}")

print()

# Print all headings so we can see section names
...
# Print raw HTML snippet so we can see structure
